...
namespace GK
{
    public partial class BanHang : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                grvBanHang.DataSource = qlbh.getTable("select * from BANHANG");
        ...
        protected void btnBanHang_Click(object sender, EventArgs e)
        {
            ...
            //}
            if (Convert.ToDateTime(txtNgayBan.Text) >= DateTime.Now)
            { 
                if (Convert.ToDateTime(txtNgayBan.Text) <= Convert.ToDateTime(txtNgayTra.Text))
                {
                    if (qlbh.checkMaMH(ddlMatHang.SelectedValue.ToString()))
                    {
                        if (qlbh.checkNgayTra(ddlMatHang.SelectedValue.ToString()) < Convert.ToDateTime(txtNgayBan.Text))
                        {
                            if (qlbh.insertBH(bh))
                            {
                                lblThongBao.Text = "thêm thành công";
                       ...
                       {
                            lblThongBao.Text = "Phòng có người đang thuê";
                       }
                    }
                    else
                    {
            ...
            {
                lblThongBao.Text = "Ngày bán phải từ ngày hôm nay";
            }
        }
